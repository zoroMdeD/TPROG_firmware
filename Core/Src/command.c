...
void read_memory()
{
	...
	// отправка по USB (проверить, не будет ли резать данные при большем количестве gpio I\O (Transmit принимает uint8_t)
...
void write_memory(uint32_t data)
{
	...
	//	uint32_t data_write = data;               // записываемые данные
		uint32_t data_write = 0xAA;               // записываем 0xАА во все ячейки памяти
